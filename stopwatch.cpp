...
StopWatch::StopWatch(QObject *parent) : QObject(parent)
{
    m_timer = NULL;
    m_runTime = QTime(0, 0);
    m_lastCumulativeTime = QTime(0, 0);
    m_timeDisplay = "00:00:00.00";

    QSettings settings(QSettings::NativeFormat, QSettings::UserScope, "BenQ", "StopWatch-CountDown");
    ...
    m_timeDisplay = settings.value("m_timeDisplay").value<QString>();
    m_runTime = settings.value("m_runTime").value<QTime>();
    m_lastCumulativeTime = settings.value("m_lastCumulativeTime").value<QTime>();
...
StopWatch::~StopWatch()
{
    ...
    settings.setValue("m_lastCumulativeTime", QVariant::fromValue(m_lastCumulativeTime));

    foreach(QObject *t, m_timeRecordList) {
        delete t;
    }
}
...
void StopWatch::addNewTimeRecord(TimeRecord *savedTimeRecord)
{
    ...
    TimeRecord *timeRecord = new TimeRecord();
    timeRecord->setLapIndex(QString("%1").arg(m_timeRecordList.size()+1, 1, 10, QChar('0')));
...
const QString &StopWatch::getTimeDisplay() const
{
